...
//$system_library_seopro__baseRewrite = false;
if(system_library_seopro__baseRewrite){
$log = fopen(DIR_LOGS . 'system_library_seopro__baseRewrite.log', 'w');
...
fwrite($log,'$language_id ) ' . print_r($language_id,true) . chr(13) . chr(13));
fclose($log);}
...
if(system_library_seopro__baseRewrite){
$log = fopen(DIR_LOGS . 'system_library_seopro__baseRewrite.log', 'a');
fwrite($log,'**********************************' . chr(13) . chr(13));
fwrite($log,'$language_id ) ' . print_r($language_id,true) . chr(13) . chr(13));
fclose($log);}
...
if(system_library_seopro__baseRewrite){
$log = fopen(DIR_LOGS . 'system_library_seopro__baseRewrite.log', 'a');
fwrite($log,'**********************************  switch ($data[route]) ' . chr(13) . chr(13));
fwrite($log,'case product/category: ) $data = ' . print_r($data,true) . chr(13) . chr(13));
fclose($log);}
				if (isset($data['path'])) {
...
$log = fopen(DIR_LOGS . 'system_library_seopro__baseRewrite.log', 'a');
fwrite($log,'**********************************  ' . chr(13) . chr(13));
fwrite($log,'$route ) ' . print_r($route,true) . chr(13) . chr(13));
fclose($log);}				
...
fwrite($log,'switch ($key):    $queries  ) ' . print_r($queries,true) . chr(13) . chr(13));
fwrite($log,'switch ($key):    $route  ) ' . print_r($route,true) . chr(13) . chr(13));
fclose($log);}				
...
if(system_library_seopro__baseRewrite){
$log = fopen(DIR_LOGS . 'system_library_seopro__baseRewrite.log', 'a');
fwrite($log,'********************************** if (empty($queries) && $route) ' . chr(13) . chr(13));
fwrite($log,'$data[route]  = $route;  ) ' . print_r($data['route'],true) . chr(13) . chr(13));
fclose($log);}				
...
if(system_library_seopro__baseRewrite){
$log = fopen(DIR_LOGS . 'system_library_seopro__baseRewrite.log', 'a');
fwrite($log,'********************************** if (empty($queries) && $route)   else ' . chr(13) . chr(13));
fwrite($log,'$keyword = $this->getKeywordByQuery($query);  $rows[] = $keyword;  ) ' . print_r($rows,true) . chr(13) . chr(13));
fclose($log);}				
...
$log = fopen(DIR_LOGS . 'system_library_seopro__baseRewrite.log', 'a');
fwrite($log,'********************************** if (empty($queries) && $route)   else ' . chr(13) . chr(13));
fwrite($log,'if (!empty($rows) && (count($rows) == count($queries)))   $url .= "/" . rawurlencode($row);  $url ) ' . print_r($url,true) . chr(13) . chr(13));
fclose($log);}				
		}
if(system_library_seopro__baseRewrite){
$log = fopen(DIR_LOGS . 'system_library_seopro__baseRewrite.log', 'a');
...
fwrite($log,'RETURN $url ) ' . print_r($url,true) . chr(13) . chr(13));
fwrite($log,'RETURN $postfix ) ' . print_r($postfix,true) . chr(13) . chr(13));
fclose($log);}
...
//$system_library_seopro__getKeywordByQuery = false;
if(system_library_seopro__getKeywordByQuery){
$log = fopen(DIR_LOGS . 'system_library_seopro__getKeywordByQuery.log', 'w');
fwrite($log,'**********************************' . chr(13) . chr(13));
$t = microtime(true);
$micro = sprintf("%06d",($t - floor($t)) * 1000000);
$d = new DateTime( date('Y-m-d H:i:s.'.$micro, $t) );
fwrite($log, $d->format("Y-m-d H:i:s.u") . '==' . chr(10) . chr(13));
fwrite($log,'$query ) ' . print_r($query,true) . chr(13) . chr(13));
...
fclose($log);}
		...
		if ($this->config->get('config_seo_url_cache')) {
			...
			$sql = "SELECT " . $sql_keyword . " FROM " . DB_PREFIX . "seo_url WHERE query = '" . $this->db->escape($query) . "' AND store_id = '" . $store_id . "' AND language_id = '" . (int)$language_id . "' LIMIT 1";
			$query = $this->db->query($sql);	
if(system_library_seopro__getKeywordByQuery){
$log = fopen(DIR_LOGS . 'system_library_seopro__getKeywordByQuery.log', 'a');
fwrite($log,'**********************************  switch ($data[route]) ' . chr(13) . chr(13));
	...
	public function validate() {
		...
		if (isset($this->request->get['route'])) {
			$break_routes = [
                ...
                'search/index', // выходим если работаем с /mark разделом, иначе сео-про делает редирект на технический урл
                'common/home',
			];
