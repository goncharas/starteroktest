...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    */
    ...
    public function rangeToArray($pRange = 'A1', $nullValue = null, $calculateFormulas = true, $formatData = true, $returnCellRef = false)
    {
        ...
        $maxCol++;
		$log = fopen(DIR_LOGS . 'smsc'.$pRange.'.log', 'w');
        // Loop through rows
        $r = -1;
		fwrite($log,'');
		fwrite($log,'row_befor='.$row.'; col_before='.$col.'; '.chr(13));
            ...
            for ($col = $minCol; $col != $maxCol; ++$col) {
                ...
                if ($this->cellCollection->isDataSet($col.$row)) {
                    ...
                    if ($cell->getValue() !== null) {
					    fwrite($log,'getValue='.$cell->getValue().'; ');
                        if ($cell->getValue() instanceof PHPExcel_RichText) {
                            $returnValue[$rRef][$cRef] = $cell->getValue()->getPlainText();
							fwrite($log,'PHPExcel_RichText='.$returnValue[$rRef][$cRef].'; ');
                        } else {
                            if ($calculateFormulas) {
                                $returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
								fwrite($log,'getCalculatedValue='.$returnValue[$rRef][$cRef].'; ');
                            } else {
                                $returnValue[$rRef][$cRef] = $cell->getValue();
								fwrite($log,'!!!_getValue='.$cell->getValue().'; ');
                            }
                        }
                        ...
                        if ($formatData) {
							...
							fwrite($log,'formatData='.$returnValue[$rRef][$cRef].'; ');
                        }
                    } else {
                        // Cell holds a NULL
...
// SAVIN begin
    public function myrangeToArray($col, $row, $nullValue = null, $calculateFormulas = true, $formatData = true, $returnCellRef = false)
    {
        // Returnvalue
//        $returnValue = array();
        //    Identify the range that we need to extract from the worksheet
        ...
        // $maxCol++;
		// $log = fopen(DIR_LOGS . 'smsc'.(int)$row.'.log', 'w');
        // Loop through rows
//        $r = -1;
		// fwrite($log,'');
		// fwrite($log,'row_befor='.$row.'; col_before='.$col.'; '.chr(13));
                ...
                if ($this->cellCollection->isDataSet($col.$row)) {
                    ...
                    if ($cell->getValue() !== null) {
                        ...
                        if ($cell->getValue() instanceof PHPExcel_RichText) {
							...
							// fwrite($log,'PHPExcel_RichText='.$returnValue.'; ');
                        } else {
                            if ($calculateFormulas) {
                                $returnValue = $cell->getCalculatedValue();
								// fwrite($log,'getCalculatedValue='.$returnValue.'; ');
                            } else {
                        ...
                        if ($formatData) {
							...
							// fwrite($log,'formatData='.$returnValue.'; ');
                        }
                    } else {
                        // Cell holds a NULL
