...
class PHPExcel_Calculation
{
    ...
    {
        if (is_string($value)) {
            if ((isset($value[0])) && ($value[0] == '"') && (substr($value, -1) == '"')) { // savin {} заменил на []
    ...
    public function parseFormula($formula)
    {
        ...
        if ((!isset($formula[0])) || ($formula[0] != '=')) { // savin {} заменил на []
            return array();
        }
        $formula = ltrim(substr($formula, 1));
        if (!isset($formula[0])) { // savin {} заменил на []
            return array();
        }
    ...
    public function _calculateFormulaValue($formula, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        if (($wsTitle1[0] !== "\x00") && ($this->cyclicReferenceStack->onStack($wsCellReference))) { // savin {} заменил на []
    ...
    private function showTypeDetails($value)
    {
        if ($this->_debugLog->getWriteDebugLog()) {
            ...
            if ($value === null) {
                ...
                } elseif ($value[0] == '#') {  // savin {} заменил на []
...
//echo 'Assessing Expression '.substr($formula, $index), PHP_EOL;
            $opCharacter = $formula[$index];   // savin {} заменил на [] //    Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter, PHP_EOL;
            if ((isset(self::$comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset(self::$comparisonOperators[$formula[$index+1]]))) { // savin {} заменил на []
                $opCharacter .= $formula[++$index];  // savin {} заменил на []
...
//                        echo 'Element '.$localeConstant.' is an Excel Constant<br />';
            ...
            //    Ignore white space
            while (($formula[$index] == "\n") || ($formula[$index] == "\r")) {  // savin {} заменил на []
                ++$index;
            }
            if ($formula[$index] == ' ') {  // savin {} заменил на []
                while ($formula[$index] == ' ') { // savin {} заменил на []
                    ++$index;
                }
...
//                    echo 'Token is a PHPExcel constant: '.$excelConstant.'<br />';
                ...
                } elseif ((is_numeric($token)) || ($token === null) || (is_bool($token)) || ($token == '') || ($token[0] == '"') || ($token[0] == '#')) { // savin {} заменил на []
...
//        }
    ...
    private function validateBinaryOperand($cellID, &$operand, &$stack)
    {
        ...
        if (is_string($operand)) {
            ...
            if ($operand > '' && $operand[0] == '"') { // savin {} заменил на []
    ...
    private function executeBinaryComparisonOperation($cellID, $operand1, $operand2, $operation, &$stack, $recursingArrays = false)
    {
        ...
        if (is_string($operand1) && $operand1 > '' && $operand1[0] == '"') { // savin {} заменил на []
            $operand1 = self::unwrapResult($operand1);
        }
        if (is_string($operand2) && $operand2 > '' && $operand2[0] == '"') { // savin {} заменил на []
