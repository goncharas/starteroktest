...
class PHPExcel_Shared_String
{
    ...
    public static function utf16_decode($str, $bom_be = true)
    {
        ...
        $c0 = ord($str[0]); // $c0 = ord($str{0}); savin
        $c1 = ord($str[1]); // $c1 = ord($str{1}); savin
        if ($c0 == 0xfe && $c1 == 0xff) {
            $str = substr($str, 2);
        ...
        for ($i=0; $i<$len; $i+=2) {
            if ($bom_be) {
                $val = ord($str[$i])   << 4; // savin {} заменил на []
                $val += ord($str[$i+1]); // savin {} заменил на []
            } else {
                $val = ord($str[$i+1]) << 4; // savin {} заменил на []
                $val += ord($str[$i]); // savin {} заменил на []
            }
            $newstr .= ($val == 0x228) ? "\n" : chr($val);
        }
        return $newstr;
