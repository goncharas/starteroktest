...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', noindex = '" . (int)$data['noindex'] . "', extrachart_id = '" . (int)$data['extrachart_id'] . "', date_modified = NOW(), date_added = NOW()");
	...
	public function editCategory($category_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', noindex = '" . (int)$data['noindex'] . "', extrachart_id = '" . (int)$data['extrachart_id'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
  ...
  public function get_partner_extra_chart(){
		$query = $this->db->query("SELECT `extrachart_id`, IF(`extrachart_default` = 1, CONCAT(`extrachart_name`,' (“по умолчанию”)'), `extrachart_name`) `extrachart_name` FROM " . DB_PREFIX . "partner_extra_chart ");
