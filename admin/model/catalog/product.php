...
class ModelCatalogProduct extends Model {
	...
	public function UpdStatusProducts() {
if(admin_model_catalog_product__UpdStatusProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__UpdStatusProducts.log', 'w');
fwrite($log,'>> ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
fclose($log);}
		$res = array();
		$sql = "UPDATE " . DB_PREFIX . "product SET status = '" . "1" . "'";
if(admin_model_catalog_product__UpdStatusProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__UpdStatusProducts.log', 'a');
fwrite($log,'>> sql) ' . print_r($sql,true) . '==' . chr(10) . chr(13));
fclose($log);}
		$this->db->query($sql);
		$res[] = $this->db->countAffected();
//		$sql = "UPDATE " . DB_PREFIX . "product p SET status = '" . "0" . "' WHERE NOT (EXISTS (SELECT 1 FROM  " . DB_PREFIX . "product_price pp WHERE pp.product_id = p.product_id AND pp.price > 0) OR EXISTS (SELECT 1 FROM  " . DB_PREFIX . "product_stock ps WHERE ps.product_id = p.product_id AND ps.quantity > 0) OR (p.image IS NOT NULL AND p.image <> ''))";
		$sql = "UPDATE " . DB_PREFIX . "product p SET status = '" . "0" . "' WHERE NOT (EXISTS (SELECT 1 FROM  " . DB_PREFIX . "product_price pp WHERE pp.product_id = p.product_id AND pp.price > 0) )";
if(admin_model_catalog_product__UpdStatusProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__UpdStatusProducts.log', 'a');
fwrite($log,'>> sql) ' . print_r($sql,true) . '==' . chr(10) . chr(13));
fclose($log);}
...
//	}
	...
	public function getProduct($product_id, $location_id = -1) {
if(admin_model_catalog_product__getProduct){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProduct.log', 'w');
fwrite($log,' ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
...
//				'stock_status'     => $query->row['stock_status'],
        ...
        'dateupdate'       => $query->row['dateupdate'],
				'viewed'           => $query->row['viewed'],
				'location_id'      => $query->row['location_id'],
				'location_name'    => $query->row['location_name'],
				'location_name1'   => $query->row['location_name1'],
        'is_partner'       => $query->row['is_partner'],
        'hasstock'         => ($query->row['stock_status'] == $stock ? '0' : '1' ),
        'shipping'         => $query->row['shipping'], // SAVIN 20220901
        'stock_status_id'  => $query->row['stock_status_id'] // SAVIN 20220901
			);
// SAVIN 20220730 beg      
if(admin_model_catalog_product__getProduct){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProduct.log', 'a');
...
// SAVIN 20220730 end      
			...
			if($ret['price'] == '1' && $ret['location_id'] == '1'){
				foreach($query->rows as $row){
				    ...
				    $ret['location_name']= $row['location_name1'];
...
if(admin_model_catalog_product__getProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProducts.log', 'w');
fwrite($log,' ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
fwrite($log,' $data )' . print_r($data, true) . '==' . chr(10) . chr(13));
fclose($log);}
		$pcur_value = $this->currency->getValue($this->config->get('config_product_currency')); // SAVIN 20220730
		$sql = "SELECT p.product_id, pd.name, ps.location_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) INNER JOIN  " . DB_PREFIX . "product_stock ps ON ps.product_id = p.product_id WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_name'])) {
			$sql .= " AND LOWER(pd.name) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')";
		}
		...
		$sql .= " GROUP BY p.product_id, ps.location_id";
...
if(admin_model_catalog_product__getProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProducts.log', 'a');
fwrite($log,' $sql )' . print_r($sql, true) . '==' . chr(10) . chr(13));
fclose($log);}
...
// SAVIN 20220730 beg
			$states = array();
			$restore_states = $this->config->get('config_restore_states');
if(admin_model_catalog_product__getProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProducts.log', 'a');
fwrite($log,' $restore_states )' . print_r($restore_states, true) . '==' . chr(10) . chr(13));
fwrite($log,' $this->session->data[language] )' . print_r((empty($this->session->data['language']) ? "ru-ru" : $this->session->data['language']), true) . '==' . chr(10) . chr(13));
fclose($log);}
			if (isset($restore_states)){
			foreach($restore_states as $rec){
				if($rec['language_code'] == (empty($this->session->data['language']) ? "ru-ru" : $this->session->data['language'])){ // $this->session->data['language']){  // SAVIN 20220817
					$states = $rec['states'];
					break;
				}
			}}
if(admin_model_catalog_product__getProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProducts.log', 'a');
fwrite($log,' $states )' . print_r($states, true) . '==' . chr(10) . chr(13));
fclose($log);}
...
if(admin_model_catalog_product__getProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProducts.log', 'a');
fwrite($log,' foreach ($query->rows as $res) )' . print_r($res, true) . '==' . chr(10) . chr(13));
fclose($log);}
		  $results = $this->getProduct($res['product_id'],$res['location_id']);
if(admin_model_catalog_product__getProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProducts.log', 'a');
fwrite($log,' $results = $this->getProduct($res[product_id]); )' . print_r($results, true) . '==' . chr(10) . chr(13));
fclose($log);}
...
if(admin_model_catalog_product__getProducts){
$log = fopen(DIR_LOGS . 'admin_model_catalog_product__getProducts.log', 'a');
fwrite($log,' $result[state] )' . print_r($result['state'], true) . '==' . chr(10) . chr(13));}
				if(!empty($result['state']) || $result['state'] == '0' ) {
if(admin_model_catalog_product__getProducts){
fwrite($log,' BEGIN $result[state] )' . print_r($result['state'], true) . '==' . chr(10) . chr(13));}
					foreach($states as $state) {
if(admin_model_catalog_product__getProducts){
fwrite($log,' if $result[state] == $state[state_id] ) ' . print_r($result['state'], true) . '==' . print_r($state, true) . chr(10) . chr(13));}
						if($state['state_id'] == $result['state']){
							$state_name = $state['name'];
if(admin_model_catalog_product__getProducts){
fwrite($log,' then $state_name )' . print_r($state_name, true) . '==' . chr(10) . chr(13));}
							break;
						}
					}
				}
if(admin_model_catalog_product__getProducts){
fwrite($log,' $state_name )' . print_r($state_name, true) . '==' . chr(10) . chr(13));
fclose($log);}
			...
			$product_data[] =  array(
				...
				'location_name1'   => $result['location_name1'],
...
// SAVIN 20220730 end
  ...
  public function getProductPrices($product_id){ 
    $sql = "SELECT opp.product_id, opp.customer_group_id, opp.price, opp.dateupdate, opp.code prc_code, opp.location_id, opp.partnerextra_id, ocgd.name cgd_name, ol.name loc_name, ol.address, ol.currency_code loc_code, ol.is_partner FROM  " . DB_PREFIX . "product_price opp LEFT JOIN " . DB_PREFIX . "customer_group_description ocgd ON ocgd.customer_group_id = opp.customer_group_id AND ocgd.language_id = '" . (int)$this->config->get('config_language_id') . "' LEFT JOIN  " . DB_PREFIX . "location ol ON opp.location_id = ol.location_id WHERE opp.product_id = '" . (int)$product_id . "' ";
    $query = $this->db->query($sql);
    return $query->rows;
  }

  public function getProductStocks($product_id){ 
    $sql = "SELECT ops.product_id, ops.location_id, ops.quantity, ops.dateupdate, ol.name loc_name, ol.address, ol.is_partner FROM " . DB_PREFIX . "product_stock ops LEFT JOIN " . DB_PREFIX . "location ol ON ops.location_id = ol.location_id WHERE ops.product_id = '" . (int)$product_id . "' ";
    $query = $this->db->query($sql);
    return $query->rows;
  }
