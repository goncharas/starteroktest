...
if(admin_model_extension_export_import__error_handler_for_export_import){
$logg = fopen(DIR_LOGS . 'admin_model_extension_export_import__error_handler_for_export_import.log', 'w');
fwrite($logg,'********************************** ' . chr(13) . chr(13));
fwrite($logg,'$config ) ' . print_r($config,true) . chr(13) . chr(13));
fwrite($logg,'$url ) ' . print_r($url,true) . chr(13) . chr(13));
fwrite($logg,'$request ) ' . print_r($request,true) . chr(13) . chr(13));
fwrite($logg,'$session ) ' . print_r($session,true) . chr(13) . chr(13));
fwrite($logg,'$log ) ' . print_r($log,true) . chr(13) . chr(13));
fclose($logg);}
    ...
    if(isset($request->get['route']) && instr($request->get['route'],'partnerprice'))
		  $link = $url->link( "catalog/partnerprice", version_compare(VERSION,'3.0','>=') ? 'user_token='.$token : 'token='.$token, true );
    else
...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$currency_id = trim($product['currency_id']);
		...
		$sql .= "`location`,`stock_status_id`,`model`,`manufacturer_id`,`image`,`shipping`,`price`,`currency_id`,`points`,`date_added`,`date_modified`,`date_available`,`weight`,`weight_class_id`,`status`,";
		...
		$sql .= in_array('isbn',$product_fields) ? "'$isbn'," : "";
		$sql .= in_array('mpn',$product_fields) ? "'$mpn'," : "";
		$sql .= "'$location',$stock_status_id,'$model',$manufacturer_id,'$image',$shipping,$price,$currency_id,$points,";
		$sql .= ($date_added=='NOW()') ? "$date_added," : "'$date_added',";
		$sql .= ($date_modified=='NOW()') ? "$date_modified," : "'$date_modified',";
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$currency_id = $this->getCell($data,$i,$j++,'1');
...
// SAVIN begin	
	protected function storePumppriceIntoDatabase( &$pumpprice_array, &$languages ) {
		$currency = $pumpprice_array['currency'];
		$model = $pumpprice_array['model'];
		$price = str_replace(',','.',$pumpprice_array['price']);
		$name = $pumpprice_array['name'];
		$quantity = $pumpprice_array['quantity'];
		$quantity = $this->IsInteger(($quantity == '' ? 0 : $quantity)) ? $quantity : 0;
		if (!isset($name)) $name = '';
		$name = ($name == '' ? '' : $this->db->escape($name));
		$pump_id = 0;
//		$this->load->model('catalog/productpump'); catalog/partnerprice
//		$store_info = $this->model_catalog_productpump->mystorePumppriceIntoDatabase( $currency, $model, $price, $name, $quantity, $pump_id );
		$this->load->model('catalog/partnerprice'); 
		$store_info = $this->model_catalog_partnerprice->mystorePumppriceIntoDatabase( $pumpprice_array );

/* 		$sql  = "INSERT INTO `".DB_PREFIX."product_pump` (`model`,`name`,`price`, `currency_id`, `date_modified`) VALUES ";
		$sql .= "('" . $this->db->escape($model) . ($name=''?"', NULL, ":"', '" . $name . "', ") . $price . ", ".(int)$currency.", NOW() );";
		$this->db->query( $sql );
		$lastId = $this->db->getLastId();
	    $sql = "UPDATE `".DB_PREFIX."product_pump` SET product_id = ifnull((SELECT product_id from `".DB_PREFIX."product` ";
		$sql .= "WHERE model = '".$this->db->escape($model)."' AND currency_id = '".(int)$currency."' LIMIT 1),(SELECT product_id from `".DB_PREFIX."product` ";
		$sql .= "WHERE replace(model,' ','') = '".str_replace(' ','',$this->db->escape($model))."' AND currency_id = '".(int)$currency."' LIMIT 1)) WHERE pump_id = '".(int)$lastId."';";
		$this->db->query( $sql );
		$sql = "UPDATE `".DB_PREFIX."product_pump` SET product_option_value_id = ifnull((SELECT product_option_value_id from `".DB_PREFIX."product_option_value` WHERE article = '".$this->db->escape($model)."' AND currency_id = '".(int)$currency."' LIMIT 1),(SELECT product_option_value_id from `".DB_PREFIX."product_option_value` WHERE replace(article,' ','') = '".str_replace(' ','',$this->db->escape($model))."' AND currency_id = '".(int)$currency."' LIMIT 1)) WHERE pump_id = '".(int)$lastId."' AND product_id is null;";
		$this->db->query( $sql );
		$sql = "UPDATE `".DB_PREFIX."product_pump pp` SET pp.product_id = (SELECT pov.product_id FROM `".DB_PREFIX."product_option_value` as pov WHERE pov.product_option_value_id = pp.product_option_value_id) WHERE pump_id = '".(int)$lastId."' AND product_id is null AND pp.product_option_value_id is not null;";
		$this->db->query( $sql );
 */	
 }
// SAVIN end	
...
// SAVIN begin
	protected function deletePumpprices($location_id) {
		$sql = "DELETE FROM `" . DB_PREFIX . "partner_price` WHERE location_id = '" . (int)$location_id . "'";
		$this->db->query( $sql );
		$sql = "DELETE FROM `" . DB_PREFIX . "product_price` WHERE location_id = '" . (int)$location_id . "'";
		$this->db->query( $sql );
		$sql = "DELETE FROM `" . DB_PREFIX . "product_stock` WHERE location_id = '" . (int)$location_id . "'";
		$this->db->query( $sql );
	}

	protected function deletePumpprice( $location_id, $model, $manufacture, $automanufacture, $automanufacture_number ) {
		$sql = "DELETE FROM `".DB_PREFIX."partner_price` WHERE location_id = '" . (int)$location_id . "' AND vendor_code_raw = '".addslashes($model)."'".(empty($manufacture) ? "" : " AND product_brand = '" . addslashes($manufacture) . "' ").(empty($automanufacture) ? "" : " AND oem_brand = '" . addslashes($automanufacture) . "' ").(empty($automanufacture_number) ? "" : " AND oem_code = '" . addslashes($automanufacture_number) . "' ");
		$this->db->query( $sql );
	}
// SAVIN end;	
...
// SAVIN begin
	protected function upload_Pumpprice( &$reader, $location_id ) {
    $currency = '';
		// if not incremental then delete all old options
	  if(empty($location_id) || (int)$location_id == 0){
	    	if ($this->$config->get('config_error_log')) 
		      $this->log->write( 'PHP (protected function upload_Pumpprice): ' . $this->language->get( 'error_empty_location_id' ));
          return false;
	  }
    $this->load->model('catalog/partnerprice'); 
		$currency = $this->model_catalog_partnerprice->getLocation2Currency( $location_id );    
	  if(empty($currency)){
	      $this->log->write( 'PHP (protected function upload_Pumpprice): ' . $this->language->get( 'error_empty_currency_code' ));
        return false;
	  }
	  $sheetname = $this->config->get( 'pumpprice_' . $location_id . '_sheetname');
		$firstrow = $this->config->get( 'pumpprice_' . $location_id . '_firstrow');
		$lastrow = $this->config->get( 'pumpprice_' . $location_id . '_lastrow');
//		$currency = $this->config->get( 'pumpprice_' . $location_id . '_currency');
		$modelclmn = $this->config->get( 'pumpprice_' . $location_id . '_modelnum');
		$priceclmn = $this->config->get( 'pumpprice_' . $location_id . '_price');
		$nameclmn = $this->config->get( 'pumpprice_' . $location_id . '_name');
    $incremental = $this->config->get( 'pumpprice_' . $location_id . '_incremental');
    
		$manufactureclmn = $this->config->get( 'pumpprice_' . $location_id . '_manufacture');
		$automanufactureclmn = $this->config->get( 'pumpprice_' . $location_id . '_automanufacture');
		$automanufacture_numberclmn = $this->config->get( 'pumpprice_' . $location_id . '_automanufacture_number');
    
    
		$quantityclmn = $this->config->get( 'pumpprice_' . $location_id . '_quantity');
		$Test_nonUniquiModelPump = $this->config->get( 'pumpprice_' . $location_id . '_nonUniquiModelPump');
// $log = fopen(DIR_LOGS . 'productpump.log', 'w');
		// fwrite($log, 'lastrow='.$lastrow.chr(13));
		// get worksheet, if not there return immediately
		...
		$this->deletePumpprices($location_id);
		

//		if (isset($currency) && !empty($currency) && $this->IsInteger($currency) && ($currency>0)) {
//		  $currency = $currency;
//		} else { return; }
		if (isset($firstrow) && !empty($firstrow) && $this->IsInteger($firstrow) && ($firstrow>0)) {
		  $beg = $firstrow;
		} else { $beg = 1; }
		if (isset($lastrow) && !empty($lastrow) && $this->IsInteger($lastrow) && ($lastrow>=$beg)) {
		  $lastrow = $lastrow;
		} else { 
		  $lastrow = $data->getHighestRow();
		}
		if (isset($quantityclmn) && !empty($quantityclmn) && $this->IsInteger($quantityclmn) && ($quantityclmn>0)) {
		  $quantityclmn = $quantityclmn;
		} else { 
		  $quantityclmn = -1;
		}
		if (isset($Test_nonUniquiModelPump) && !empty($Test_nonUniquiModelPump) && $this->IsInteger($Test_nonUniquiModelPump) && ($modelclmn>0)) {
		  $Test_nonUniquiModelPump_ = true;
		} else { $Test_nonUniquiModelPump_ = false; }
		if (isset($modelclmn) && !empty($modelclmn) && $this->IsInteger($modelclmn) && ($modelclmn>0)) {
		  $modelclmn = $modelclmn;
		} else { $modelclmn=1;}
		if (isset($priceclmn) && !empty($priceclmn) && $this->IsInteger($priceclmn) && ($priceclmn>0)) {
		  $priceclmn = $priceclmn;
		} else { return;}
		if (isset($nameclmn) && !empty($nameclmn) && $this->IsInteger($nameclmn) && ($nameclmn>0)) {
		  $nameclmn = $nameclmn;
		} else { $nameclmn=-1;}

		if (isset($manufactureclmn) && !empty($manufactureclmn) && $this->IsInteger($manufactureclmn) && ($manufactureclmn>0)) {
		  $manufactureclmn = $manufactureclmn;
		} else { $manufactureclmn=-1;}
		if (isset($automanufactureclmn) && !empty($automanufactureclmn) && $this->IsInteger($automanufactureclmn) && ($automanufactureclmn>0)) {
		  $automanufactureclmn = $automanufactureclmn;
		} else { $automanufactureclmn=-1;}
		if (isset($automanufacture_numberclmn) && !empty($automanufacture_numberclmn) && $this->IsInteger($automanufacture_numberclmn) && ($automanufacture_numberclmn>0)) {
		  $automanufacture_numberclmn = $automanufacture_numberclmn;
		} else { $automanufacture_numberclmn=-1;}


		$this->load->model('catalog/partnerprice');
		// load the worksheet cells and store them to the database
$log = fopen(DIR_LOGS . 'partnerprice.log', 'w');
fwrite($log, 'BEGIN  ' . 'firstrow=' . $firstrow . '; lastrow=' . $lastrow . '  ' . gmdate('d.m.Y  H:i:s') . chr(10) . chr(13));
		for ($i=$beg; $i<=$lastrow; $i+=1) {
			$model = trim($this->mygetCell($data,$i,$modelclmn,''));
			$price = trim($this->mygetCell($data,$i,$priceclmn, ''));
			if ($nameclmn>0) {
			  $name=htmlspecialchars(trim($this->mygetCell($data,$i,$nameclmn, '')));
			} else { $name = ''; }
      
			if ($manufactureclmn>0) {
			  $manufacture=htmlspecialchars(trim($this->mygetCell($data,$i,$manufactureclmn, '')));
			} else { $manufacture = ''; }
			if ($automanufactureclmn>0) {
			  $automanufacture=htmlspecialchars(trim($this->mygetCell($data,$i,$automanufactureclmn, '')));
			} else { $automanufacture = ''; }
			if ($automanufacture_numberclmn>0) {
			  $automanufacture_number=htmlspecialchars(trim($this->mygetCell($data,$i,$automanufacture_numberclmn, '')));
			} else { $automanufacture_number = ''; }
      
      
      
			if ($quantityclmn>0) {
			  $quantity=trim($this->mygetCell($data,$i,$quantityclmn, ''));
			} else { $quantity = ''; }
fwrite($log, $i . '. model='. $model . '; currency=' . $currency . '; price=' . $price . '; name=' . $name . '; quantity=' . $quantity . chr(10) . chr(13));
			 if (($model=='') || ($price == '') || ($model == $price) || !($this->IsInteger(str_replace('.','',str_replace(',','',$price))))) {
//			if (($model=='') || ($price == '') || ($model == $price)) {				
			   continue;
			}
			if ($incremental) {
				$this->deletePumpprice( $location_id, $model, $manufacture, $automanufacture, $automanufacture_number );
			}
			$pumpprice_array = array();
			$pumpprice_array['currency'] = $currency;
			$pumpprice_array['location_id'] = $location_id;
			$pumpprice_array['price'] = $price;
			$pumpprice_array['name']  = $name;
			$pumpprice_array['model'] = $model;
			$pumpprice_array['manufacture']  = $manufacture;
			$pumpprice_array['automanufacture']  = $automanufacture;
			$pumpprice_array['automanufacture_number']  = $automanufacture_number;
			$pumpprice_array['quantity']  = $quantity;
//			$this->moreOptionCells( $i, $j, $data, $option );
//			$this->storePumppriceIntoDatabase( $pumpprice_array, $languages );
      
		$store_info = $this->model_catalog_partnerprice->mystorePumppriceIntoDatabase( $pumpprice_array );
      
		}
fwrite($log, 'END' . chr(10) . chr(13));		
fclose($log);	

		$this->load->model('catalog/partnerprice');
	  $this->model_catalog_partnerprice->updateStatus4Double( $location_id ); 
	}

	protected function upload_Pumpprice_csv( &$reader, $location_id ) {
    $currency = '';
		// if not incremental then delete all old options
	  if(empty($location_id) || (int)$location_id == 0){
	    	if ($this->$config->get('config_error_log')) 
...
fwrite($log, chr(10) . chr(13) . '******************************* upload_Pumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $automanufactureclmn) ' . (isset($automanufactureclmn) ? print_r($automanufactureclmn,true) : 'NOT ISSET $automanufactureclmn') . '==' . chr(10) . chr(13));
fwrite($log,' isset($automanufactureclmn) ) ' . (isset($automanufactureclmn) ? 'TRUE' : 'FALSE') . '==' . chr(10) . chr(13));
fwrite($log,' !empty($automanufactureclmn) ) ' . (!empty($automanufactureclmn) ? 'TRUE' : 'FALSE') . '==' . chr(10) . chr(13));
fwrite($log,' $this->IsInteger($automanufactureclmn)) ' . ($this->IsInteger($automanufactureclmn) ? 'TRUE' : 'FALSE') . '==' . chr(10) . chr(13));
fwrite($log,' $quantityclmn) ' . (isset($quantityclmn) ? print_r($quantityclmn,true) : 'NOT ISSET $quantityclmn') . '==' . chr(10) . chr(13));
fclose($log);}
		$Test_nonUniquiModelPump = $this->config->get( 'pumpprice_' . $location_id . '_nonUniquiModelPump');
...
//		}
    ...
    $quantityclmns = array();
    if (isset($quantityclmn) && !empty($quantityclmn)){
        $expl = explode('+',$quantityclmn);
        foreach($expl as $rec)
          if($this->IsInteger($rec) && ($rec>0))
            $quantityclmns[] = $rec;
		}
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* upload_Pumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $expl) ' . (isset($expl) ? print_r($expl,true) : 'NOT ISSET $expl') . '==' . chr(10) . chr(13));
fwrite($log,' $quantityclmns) ' . (isset($quantityclmns) ? print_r($quantityclmns,true) : 'NOT ISSET $quantityclmns') . '==' . chr(10) . chr(13));
fwrite($log,' $beg) ' . (isset($beg) ? print_r($beg,true) : 'NOT ISSET $beg') . '==' . chr(10) . chr(13));
fwrite($log,' $lastrow) ' . (isset($lastrow) ? print_r($lastrow,true) : 'NOT ISSET $lastrow') . '==' . chr(10) . chr(13));
fclose($log);}
    
		if (isset($Test_nonUniquiModelPump) && ($Test_nonUniquiModelPump !== '') && $this->IsInteger($Test_nonUniquiModelPump) && ($modelclmn>=0)) {
		  $Test_nonUniquiModelPump_ = $Test_nonUniquiModelPump == '1' ? true : false ;
		} else { $Test_nonUniquiModelPump_ = false; }
		if (isset($modelclmn) && ($modelclmn !== '') && $this->IsInteger($modelclmn) && ($modelclmn>=0)) {
		  $modelclmn = $modelclmn;
		} else { $modelclmn=1;}
		if (isset($priceclmn) && ($priceclmn !== '') && $this->IsInteger($priceclmn) && ($priceclmn>=0)) {
		  $priceclmn = $priceclmn;
		} else { return;}
		if (isset($nameclmn) && ($nameclmn !== '') && $this->IsInteger($nameclmn) && ($nameclmn>=0)) {
		  $nameclmn = $nameclmn;
		} else { $nameclmn=-1;}

		if (isset($manufactureclmn) && ($manufactureclmn !== '') && $this->IsInteger($manufactureclmn) && ($manufactureclmn>=0)) {
		  $manufactureclmn = $manufactureclmn;
		} else { $manufactureclmn=-1;}
		if (isset($automanufactureclmn) && ($automanufactureclmn !== '') && $this->IsInteger($automanufactureclmn) && ($automanufactureclmn>=0)) {
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* upload_Pumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $automanufactureclmn) ' . (isset($automanufactureclmn) ? print_r($automanufactureclmn,true) : 'NOT ISSET $automanufactureclmn') . '==' . chr(10) . chr(13));
fclose($log);}
		...
		if (isset($automanufacture_numberclmn) && ($automanufacture_numberclmn !== '') && $this->IsInteger($automanufacture_numberclmn) && ($automanufacture_numberclmn>=0)) {
		  $automanufacture_numberclmn = $automanufacture_numberclmn;
		} else { $automanufacture_numberclmn=-1;}
		...
		$this->load->model('catalog/partnerprice');
		// load the worksheet cells and store them to the database
$logg = fopen(DIR_LOGS . 'partnerprice.log', 'w');
fwrite($logg, 'BEGIN  ' . 'firstrow=' . $firstrow . '; lastrow=' . $lastrow . '  ' . gmdate('d.m.Y  H:i:s') . chr(10) . chr(13));

   $row = 1;
   while (($data = fgetcsv($reader, 1000, $sheetname)) !== FALSE) {
     $num = count($data);
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* upload_Pumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $row) ' . (isset($row) ? print_r($row,true) : 'NOT ISSET $row') . '==' . chr(10) . chr(13));
fwrite($log,' $num) ' . (isset($num) ? print_r($num,true) : 'NOT ISSET $num') . '==' . chr(10) . chr(13));
fclose($log);}
     $model = ''; $price = ''; $name = ''; $manufacture = ''; 
     $automanufacture = ''; $automanufacture_number = ''; $quantity = '';  
     if($row >= $beg && ($lastrow == -1 || $row <= $lastrow) && $num > 0){
       for ($c=0; $c < $num; $c++) {
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* upload_Pumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $c) ' . (isset($c) ? print_r($c,true) : 'NOT ISSET $c') . '==' . chr(10) . chr(13));
fwrite($log,' $data[$c]) ' . (isset($data[$c]) ? print_r($data[$c],true) : 'NOT ISSET $data[$c]') . '==' . chr(10) . chr(13));
fclose($log);}
         if($c == $modelclmn){
           $model = trim($data[$c]);
         } else if($modelclmn > $num) {
           $model = ''; 
         }
         if($c == $priceclmn){
           $price = trim($data[$c]);
         } else if($priceclmn > $num) {
           $price = ''; 
         }
         if($c == $nameclmn){
           $name = htmlspecialchars(trim($data[$c]));
         } else if($nameclmn > $num || $nameclmn < 0) {
           $name = ''; 
         }
         if($c == $manufactureclmn){
           $manufacture = htmlspecialchars(trim($data[$c]));
         } else if($manufactureclmn > $num || $manufactureclmn < 0) {
           $manufacture = ''; 
         }
         if($c == $automanufactureclmn){
           $automanufacture = htmlspecialchars(trim($data[$c]));
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* upload_Pumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $c == $automanufactureclmn) ' . (isset($automanufactureclmn) ? print_r($automanufactureclmn,true) : 'NOT ISSET $automanufactureclmn') . '==' . chr(10) . chr(13));
fwrite($log,' $automanufacture) ' . (isset($automanufacture) ? print_r($automanufacture,true) : 'NOT ISSET $automanufacture') . '==' . chr(10) . chr(13));
fclose($log);}
         } else if($automanufactureclmn > $num || $automanufactureclmn < 0) {
           $automanufacture = ''; 
         }
         if($c == $automanufacture_numberclmn){
           $automanufacture_number = htmlspecialchars(trim($data[$c]));
         } else if($automanufacture_numberclmn > $num || $automanufacture_numberclmn < 0) {
           $automanufacture_number = ''; 
         }
         if(in_array($c,$quantityclmns)){
//           $quant = htmlspecialchars(trim($data[$c]));
           $quant = trim($data[$c]);
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* upload_Pumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $quantity) ' . (isset($quantity) ? print_r($quantity,true) : 'NOT ISSET $quantity') . '==' . chr(10) . chr(13));
fwrite($log,' $quant) ' . (isset($quant) ? print_r($quant,true) : 'NOT ISSET $quant') . '==' . chr(10) . chr(13));
fclose($log);}
           if($quant !== ''){
             if($quantity == ''){
               $quantity = $quant;
...
//         if($c == $quantityclmn){
//           $quantity = htmlspecialchars(trim($data[$c]));
//         } else if($quantityclmn > $num || $quantityclmn < 0) {
//           $quantity = '0'; 
//         }
       }

fwrite($logg, $row . '. model='. $model . '; currency=' . $currency . '; price=' . $price . '; name=' . $name . '; quantity="' . $quantity . '"' . chr(10) . chr(13));
			 if (($quantity=='') || ($model=='') || ($price == '') || ($model == $price) || !($this->IsInteger(str_replace('.','',str_replace(',','',$price))))) {
//			if (($model=='') || ($price == '') || ($model == $price)) {				
			   continue;
			}
			if ($incremental) {
...
// SAVIN begin
     ...
     *    @return    string
     */
    protected function mystringFromColumnIndex($pColumnIndex = 0)  // перенес из Cell.php
    {
	...
	protected function mygetCell(&$worksheet,$row,$col,$default_val='*') {
		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index
//		$row += 1; // we use 0-based, PHPExcel uses 1-based row index
	    // $log = fopen(DIR_LOGS . 'smsc.log'.(int)$row, 'w');
		if ($worksheet->cellExistsByColumnAndRow($col,$row)) {
		  // $val =  $worksheet->getCellByColumnAndRow($col,$row)->getValue();
		  $coln = $this->mystringFromColumnIndex($col);
//		  $myCell = $coln.(int)$row;
		  // fwrite($log, 'col=<'.(int)$col.'> ; row='.(int)$row.'; Cell=~'.$myCell.'~; ');
		  $val = $worksheet->myrangeToArray($coln,$row, null, true, true, true);
		} else { 
		  $val =  $default_val;
		}
		if ($val===null) {
...
// SAVIN begin
	protected function validatePumpprice( &$reader, $location_id ) {
		$sheetname = $this->config->get( 'pumpprice_' . $location_id . '_sheetname');
		$data = $reader->getSheetByName( $sheetname );
		if ($data==null) {
			return false;
...
// SAVIN end
	...
	protected function validateProducts( &$reader ) {
		...
		if ($this->use_table_seo_url) {
			$expected_heading = array_merge( $expected_heading, array( "location", "quantity", "model", "manufacturer", "image_name", "shipping", "price", "currency_id", "points", "date_added", "date_modified", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status", "tax_class_id", "description") );
		} else {
			$expected_heading = array_merge( $expected_heading, array( "location", "quantity", "model", "manufacturer", "image_name", "shipping", "price", "currency_id", "points", "date_added", "date_modified", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status", "tax_class_id", "seo_keyword", "description") );
		}
		if ($exist_meta_title) {
...
// SAVIN begin
	protected function validateProductIdColumnsPumpprice( &$reader, $location_id ) {
	  if(empty($location_id) || (int)$location_id == 0){
	    	if ($config->get('config_error_log')) 
		      $this->log->write( 'PHP (protected function validateProductIdColumnsPumpprice): ' . $this->language->get( 'error_empty_location_id' ));
          return false;
	  }
    $this->load->model('catalog/partnerprice'); 
		$currency = $this->model_catalog_partnerprice->getLocation2Currency( $location_id );    
	  if(empty($currency)){
	      $this->log->write( 'PHP (protected function validateProductIdColumnsPumpprice): ' . $this->language->get( 'error_empty_currency_code' ));
        return false;
	  }
    ...
    $sheetname = $this->config->get( 'pumpprice_' . $location_id . '_sheetname');
		...
		$product_ids = array();
		if (isset($firstrow) && ($firstrow !== '') && $this->IsInteger($firstrow) && ($firstrow>0)) {
		  $beg = $firstrow;
		} else { $beg = 1; }
		if (isset($lastrow) && ($lastrow !== '') && $this->IsInteger($lastrow) && ($lastrow>$beg)) {
		  $k = $lastrow;
		} else { 
		  $k = $data->getHighestRow();
		}
...
//		$log = fopen(DIR_LOGS . 'smsc.log', 'w');
		for ($i=$beg; $i<=$k; $i+=1) {
		    // fwrite($log,'i='.(int)$i.'; j='.(int)$j);
			$product_id = $this->mygetCell($data,$i,$j);
			// fwrite($log,'; from rows='.(int)$k.'; getCell='.$product_id.chr(13));
			// if ($product_id=="") {
				// if (!$has_missing_product_ids) {
					// $msg = str_replace( '%1', $sheetname, $this->language->get( 'error_missing_product_id' ) );
					// $this->log->write( $msg );
				...
				// continue;
			// }
			// if (trim($product_id)==$product_id) {
				// $msg = str_replace( '%2', $product_id, str_replace( '%1', $sheetname, $this->language->get( 'error_invalid_product_id' ) ) );
				// $this->log->write( $msg );
				// $ok = false;
				// continue;
			// }
			if ($Test_nonUniquiModelPump_ && (in_array( $product_id, $product_ids ))) {
				$msg = str_replace( '%2', $product_id, str_replace( '%1', $sheetname, $this->language->get( 'error_duplicate_model' ) ) );
				$this->log->write( $msg );
				$ok = false;
			}
			$product_ids[] = $product_id;
//			if ($product_id < $previous_product_id) {
//				$msg = str_replace( '%2', $product_id, str_replace( '%1', $sheetname, $this->language->get( 'error_wrong_order_product_id' ) ) );
//				$this->log->write( $msg );
//				$ok = false;
//			}
			$previous_product_id = $product_id;
	...
	protected function validateProductIdColumnsPumpprice_csv( $reader, $location_id ) {
	  if(empty($location_id) || (int)$location_id == 0){
	    	if ($config->get('config_error_log')) 
		      $this->log->write( 'PHP (protected function validateProductIdColumnsPumpprice): ' . $this->language->get( 'error_empty_location_id' ));
          return false;
...
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log, chr(10) . chr(13) . '******************************* validateProductIdColumnsPumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $Test_nonUniquiModelPump) ' . (isset($Test_nonUniquiModelPump) ? print_r($Test_nonUniquiModelPump,true) : 'NOT ISSET $Test_nonUniquiModelPump') . '==' . chr(10) . chr(13));
fwrite($log,' isset($Test_nonUniquiModelPump) ) ' . (isset($Test_nonUniquiModelPump) ? 'TRUE' : 'FALSE') . '==' . chr(10) . chr(13));
fwrite($log,' !empty($Test_nonUniquiModelPump) ) ' . (!empty($Test_nonUniquiModelPump) ? 'TRUE' : 'FALSE') . '==' . chr(10) . chr(13));
fwrite($log,' $this->IsInteger($Test_nonUniquiModelPump)) ' . ($this->IsInteger($Test_nonUniquiModelPump) ? 'TRUE' : 'FALSE') . '==' . chr(10) . chr(13));
fwrite($log,' $modelclmn) ' . (isset($modelclmn) ? print_r($modelclmn,true) : 'NOT ISSET $modelclmn') . '==' . chr(10) . chr(13));
fclose($log);}
		if (isset($Test_nonUniquiModelPump) && ($Test_nonUniquiModelPump !='') && $this->IsInteger($Test_nonUniquiModelPump) && ($modelclmn>=0)) {
		  $Test_nonUniquiModelPump_ = $Test_nonUniquiModelPump == '1' ? true : false ;
		} else { $Test_nonUniquiModelPump_ = false; }
...
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log, chr(10) . chr(13) . '******************************* validateProductIdColumnsPumpprice_csv ' . '==' . chr(10) . chr(13));
fwrite($log,' $Test_nonUniquiModelPump_ ) ' . ($Test_nonUniquiModelPump_ ? 'TRUE' : 'FALSE') . '==' . chr(10) . chr(13));
fwrite($log,' IF $Test_nonUniquiModelPump_ == TRUE >> осуществляем проверку моделей на уникальность перед закачкой. Закачка останавливается если есть НЕ уникальные записи Модели. Результат проверки в файле лога. ) ' . '==' . chr(10) . chr(13));
fclose($log);}
    if (!$Test_nonUniquiModelPump_)
      return true;
		$ok = true;
...
$logg = fopen(DIR_LOGS . 'partnerprice.log', 'w');
fwrite($logg, 'BEGIN TEST uniq models ' . 'firstrow=' . $firstrow . '; lastrow=' . $lastrow . '  ' . gmdate('d.m.Y  H:i:s') . chr(10) . chr(13));
...
//        echo "<p> $num полей в строке $row: <br /></p>\n";
      if($row >= $beg && ($k == -1 || $row <= $k)) {
         for ($c=0; $c < $num; $c++) {
//            echo $data[$c] . "<br />\n";
          if($c == $j)
            $product_id = $data[$c]; 
         }
			   if ($Test_nonUniquiModelPump_ && ($product_id !== '') && (in_array( $product_id, $product_ids ))) {
				    $msg = str_replace( '%2', $product_id, str_replace( '%1', $row, $this->language->get( 'error_duplicate_model_csv' ) ) );
				    $this->log->write( $msg );
fwrite($logg, $row . '. '. $msg . chr(10) . chr(13));
				    $ok = false;
			   } else {
			     $product_ids[] = $product_id;
         }
...
// SAVIN begin
	protected function validateIncrementalOnlyPumpprice( &$reader, $incremental ) {
		// certain worksheets can only be imported in incremental mode for the time being
		$ok = true;
		$sheetname = $this->config->get( 'pumpprice_sheetname');
		$worksheets = array( $sheetname );
		foreach ($worksheets as $worksheet) {
			$data = $reader->getSheetByName( $worksheet );
			if ($data) {
				if (!$incremental) {
...
// SAVIN begin
	protected function validateWorksheetNamesPumpprice( &$reader, $location_id ) {
	  $sheetname = $this->config->get('pumpprice_' . $location_id . '_sheetname');
		$allowed_worksheets = array($sheetname);
		$all_worksheets_ignored = true;
		$worksheets = $reader->getSheetNames();
		foreach ($worksheets as $worksheet) {
			if (in_array($worksheet,$allowed_worksheets)) {
	...
	protected function validateWorksheetNamesPumpprice_csv( &$reader, $location_id ) {
	  $sheetname = $this->config->get('pumpprice_' . $location_id . '_sheetname');
		return !empty($sheetname);
	}


	protected function validateUploadPumpprice( &$reader, $location_id )
	{
		$ok = true;
		$languages = $this->getLanguages();
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* before if (!$this->validateWorksheetNamesPumpprice( $reader ))' . '==' . chr(10) . chr(13));
fclose($log);}

		// make sure at least one of worksheet names is valid
		if (!$this->validateWorksheetNamesPumpprice( $reader, $location_id )) {
...
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* before if (!$this->validatePumpprice( $reader ))' . '==' . chr(10) . chr(13));
fclose($log);}
		if (!$this->validatePumpprice( $reader, $location_id )) {
			$this->log->write( $this->language->get('error_worksheets')); // 'error_categories_header') );
			$ok = false;
		}
		// certain worksheets rely on the existence of other worksheets
		$names = $reader->getSheetNames();
		$exist_pumpprice = false;
		$sheetname = $this->config->get( 'pumpprice_' . $location_id . '_sheetname');
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* $names = $reader->getSheetNames();  ' . '==' . chr(10) . chr(13));
fwrite($log,'>> $names) ' . print_r($names,true) . '==' . chr(10) . chr(13));
fwrite($log,'>> $sheetname = $this->config->get(pumpprice_' . $location_id . '_sheetname);  ) ' . print_r($sheetname,true) . '==' . chr(10) . chr(13));
fclose($log);}
		foreach ($names as $name) {
			if ($name==$sheetname) {
				$exist_pumpprice = true;
				continue;
			}
		}
		if (!$ok) {
...
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* before if (!$this->validateProductIdColumnsPumpprice( $reader ))' . '==' . chr(10) . chr(13));
fclose($log);}
		if (!$this->validateProductIdColumnsPumpprice( $reader, $location_id )) {
			$ok = false;
		}
...
fwrite($log,'>> return $ok ) ' . print_r($ok ? 'TRUE' : 'FALSE',true) . '==' . chr(10) . chr(13));
fclose($log);}
		return $ok;
	}
  
	protected function validateUploadPumpprice_csv( &$reader, $location_id )
	{
		$ok = true;
		$languages = $this->getLanguages();
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* before if (!$this->validateWorksheetNamesPumpprice_csv( $reader ))' . '==' . chr(10) . chr(13));
fclose($log);}

		// make sure at least one of worksheet names is valid
		if (!$this->validateWorksheetNamesPumpprice_csv( $reader, $location_id )) {
			$this->log->write( $this->language->get( 'error_worksheets_csv' ) );
			$ok = false;
		}

if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* before if (!$this->validateProductIdColumnsPumpprice_csv( $reader ))' . '==' . chr(10) . chr(13));
fclose($log);}
		if (!$this->validateProductIdColumnsPumpprice_csv( $reader, $location_id )) {
			$ok = false;
		}
...
// SAVIN begin
	public function uploadpumpprice( $filename, $location_id) { //, $incremental=false ) {
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'w');
fwrite($log,'>> ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
fwrite($log,'>> $filename) ' . print_r($filename,true) . '==' . chr(10) . chr(13));
fwrite($log,'>> $location_id) ' . print_r($location_id,true) . '==' . chr(10) . chr(13));
// fwrite($log,'>> $incremental) ' . print_r($incremental ? 'TRUE' : 'FALSE',true) . '==' . chr(10) . chr(13));
fclose($log);}
		...
		try {
			...
			// if (!$this->validateIncrementalOnlyPumpprice( $reader, $incremental )) {
				// return false;
			// }
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* before if (!$this->validateUploadPumpprice( $reader ))' . '==' . chr(10) . chr(13));
fwrite($log,'>> $reader) ' . isset($reader) ? 'NOT ISSET($READER)' : print_r($reader,true) . '==' . chr(10) . chr(13));
fclose($log);}
			if (!isset($reader) || !$this->validateUploadPumpprice( $reader, $location_id )) {
				return false;
			}
			$this->clearCache();
			$this->session->data['export_import_nochange'] = 0;
			$this->upload_Pumpprice( $reader, $location_id); //, $incremental );
			return true;
		} catch (Exception $e) {
			$errstr = $e->getMessage();
			$errline = $e->getLine();
...
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'a');
fwrite($log,'******************************* catch (Exception $e) ' . '==' . chr(10) . chr(13));
fwrite($log,'>>  ' . print_r('PHP ' . get_class($e) . ':  ' . $errstr . ' in ' . $errfile . ' on line ' . $errline,true) . '==' . chr(10) . chr(13));
fwrite($log,'>>  ' . print_r($this->session->data['export_import_error'],true) . '==' . chr(10) . chr(13));
fclose($log);}
			if ($this->config->get('config_error_log')) {
	...
	public function uploadpumpprice_csv( $filename, $location_id) { //, $incremental=false ) {
if(admin_model_extension_export_import__uploadpumpprice){
$log = fopen(DIR_LOGS . 'admin_model_extension_export_import__uploadpumpprice.log', 'w');
fwrite($log,'>> ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
fwrite($log,'>> $filename) ' . print_r($filename,true) . '==' . chr(10) . chr(13));
...
fwrite($log,'>> $reader) ' . isset($reader) ? 'NOT ISSET($READER)' : print_r($reader,true) . '==' . chr(10) . chr(13));
fclose($log);}
			if (!isset($reader) || !$this->validateUploadPumpprice_csv( $reader, $location_id )) {
			  fclose($reader);
				return false;
			}
		  fclose($reader);
      $reader = fopen($filename,'r'); // Не знаю, как после проверки переместиться снова в начало файла, поэтому переоткрываю его
			$this->upload_Pumpprice_csv( $reader, $location_id); //, $incremental );
      fclose($reader);
			return true;
		} catch (Exception $e) {
      fclose($reader);
...
//		}
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('currency_id'),5)+1);
...
// SAVIN begin
	public function mygetNotifications() {
//		$language_code = $this->config->get( 'config_admin_language' );
		$result = "Добро пожаловать в форму закачки цен из ексель файла через промежуточную таблицу в таблицу товаров для OpenCart. Если вам надо доработка формы, обращайтесь  <a href='http://www.entechit.com.ua/#team'><u>к команде разработчиков</u></a>. Рады будем Вам помочь.";
		return $result;
	}
// SAVIN end	

	public function getModelPumpNameCounts($location_id) {
		$sql  = "SELECT p.`model`, COUNT(p.product_id) AS `count` FROM `".DB_PREFIX."product` p INNER JOIN `".DB_PREFIX."product_price` pp ON pp.product_id = p.product_id AND pp.location_id = '" . (int)$location_id . "' AND pp.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' ";
