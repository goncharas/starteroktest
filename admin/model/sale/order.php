...
class ModelSaleOrder extends Model {
	...
	public function getOrderProducts($order_id) {
	  $sql = "SELECT op.*, TRIM(LEADING ',' FROM IF(ld.city IS NOT NULL AND ld.name IS NOT NULL, CONCAT(ld.city,', ',ld.name),IF(ld.city IS NOT NULL, ld.city, ld.name)))  as location FROM " . DB_PREFIX . "order_product op  LEFT JOIN " . DB_PREFIX . "location_description ld ON ld.location_id = op.location_id  WHERE op.order_id = '" . (int)$order_id . "'  AND IFNULL(ld.language_id,'" . (int)$this->config->get('config_language_id') . "') = '" . (int)$this->config->get('config_language_id') . "'"; 
if(admin_model_sale_order__getOrderProducts){
$log = fopen(DIR_LOGS . 'admin_model_sale_order__getOrderProducts.log', 'w');
