...
if(admin_controller_extension_export_import_export_import__upload){
$log = fopen(DIR_LOGS . 'admin_controller_extension_export_import_export_import__upload.log', 'w');
...
if(admin_controller_extension_export_import_export_import__upload){
$log = fopen(DIR_LOGS . 'admin_controller_extension_export_import_export_import__upload.log', 'a');
fwrite($log,'********************************** after  if (($this->request->server[REQUEST_METHOD] == POST) && ($this->validateUploadForm()))' . chr(13) . chr(13));
fclose($log);}
...
fclose($log);}
		       ...
		       if (($this->request->server['REQUEST_METHOD'] == 'POST') && (!empty($this->request->post['location_id'])) && ($this->request->post['location_id'] == '1001')) {
if(admin_controller_extension_export_import_export_import__upload){
$log = fopen(DIR_LOGS . 'admin_controller_extension_export_import_export_import__upload.log', 'a');
fwrite($log,'********************************** after  if (($this->request->server[REQUEST_METHOD] == POST) && (!empty($this->request->post[location_id])) && ($this->request->post[location_id] == 1001))' . chr(13) . chr(13));
fclose($log);}
		          $res = $this->model_catalog_partnerprice->deletedoubleprices($this->request->post['location_id']);
           }
		      $this->load->model('extension/export_import');
	...
	public function getNotifications() {
		...
		$this->load->model('extension/export_import');
		$this->load->language( 'extension/export_import' );
		$response = $this->model_extension_export_import->mygetNotifications();
		$json = array();
		if ($response===false) {
			$json['message'] = '';
