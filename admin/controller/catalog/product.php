...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		*/
		...
		$data['upd_status_products'] = $this->url->link('catalog/product/upd_status_products', 'user_token=' . $this->session->data['user_token'] . $url, true);
    ...
    $data['partnerprice_list'] = $this->model_catalog_product->getProductPrices($this->request->get['product_id']);
    $data['productstock_list'] = $this->model_catalog_product->getProductStocks($this->request->get['product_id']);
	...
	public function upd_status_products() {
$log = fopen(DIR_LOGS . 'admin_controller_catalog_product__upd_status_products.log', 'w');
fwrite($log,'>> ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
fclose($log);
		$json = array();
        $this->load->language('catalog/product');
        $this->load->model('catalog/product');
		$res = $this->model_catalog_product->UpdStatusProducts();
		// 'Сброшен статус в "Включено" у %s товаров. Заново статус в "Отключен" установлено у %s строк'
		$json[] = sprintf($this->language->get('result_upd'), (isset($res[0]) ? ''.$res[0] : '?'), (isset($res[1]) ? ''.$res[1] : '?')); //	'UPDATED 6 records';
$log = fopen(DIR_LOGS . 'admin_controller_catalog_product__upd_status_products.log', 'a');
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					...
					'label'      => strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')).'; '.(empty($result['manufacturer']) ? '' : ($result['manufacturer'].'; ')).(empty($result['state_name']) ? '' : ($result['state_name'].'; ')).(empty($result['quantity']) ? '' : (' quantity='.$result['quantity'].'; ')).(empty($result['dateupdate']) ? '' : ($result['dateupdate'].'; ')).(empty($result['price']) ? '' : (' price='.$result['price'].'; ')).(empty($result['stock_status']) ? '' : ($result['stock_status'].'; ')), // SAVIN  20220730
//					'label'      => strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')), // SAVIN  20220730
					'model'      => $result['model'],
					'option'     => $option_data,
          'location'   => $result['location'], // SAVIN  20220730
          'location_id'=> $result['location_id'], // SAVIN  20220730
          'stock_status'=> $result['stock_status'] , // SAVIN  20220730
          'is_partner' => $result['is_partner'], // SAVIN  20220730
          'hasstock'   => ($result['stock_status'] == $stock ? '0' : '1' ), // SAVIN  20220730
          'status'     => $result['status'], // SAVIN  20220730
          'manufacturer'=> $result['manufacturer'], // SAVIN  20220730
          'state_name' => $result['state_name'], // SAVIN  20220730
          'dateupdate' => $result['dateupdate'], // SAVIN  20220730
          'sku'        => $result['sku'], // SAVIN  20220730
					'price'      => $result['price']
/*          
							<td class="">{{ pl.manufacturer }}</td>
							<td class="">{{ pl.state_name }}</td>
							<td class="">{{ pl.quantity }}</td>
							<td class="">{{ pl.dateupdate }}</td>
							<td class="">{{ pl.price }}</td>
							<td class="">{{ pl.stock_status }}</td>
*/          
