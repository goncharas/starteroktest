...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		$data['text_main_office'] = $this->language->get('text_main_office');

		$data['locations'] = array();
		$data['location_list'] = array();
		$data['location_city_list'] = array();
		$city = '';
		$city_link = '';
		$city_id = 'c1';
		$department_name = '';
		...
		foreach ($this->model_localisation_location->getLocationList($this->config->get('config_language_id')) as $result) {
			if ($result['location_id'] == 1) {
                $data['location_top_address'] = $this->language->get('text_prefix_address') . $result['city'] . ', ' . nl2br($result['address']);
			} 

			if ($loc_id != $result['location_id']) {
			    $department_name = '';	
			}

			if ($city != $result['city']) {
				$city_link = $_SERVER["REQUEST_URI"].'#c' . $i;
				$city_id = 'c' . $i;
				++$i;
            } 
            ...
            foreach ($this->model_localisation_location->getStuffList($result['location_id'], $this->config->get('config_language_id')) as $res) {
            	...
            	if ($department_name != $res['department_name']) {
            	...
            	if ($res['image']) {
					$image = $this->model_tool_image->resize($res['image'], 100, 100);
				} else {
	        ...
	        $data['location_list'][] = array(
			    ...
			    'city_id'   => $city_id,
			    'name'        => $result['name'],
				'address'     => $this->language->get('text_prefix_address') . $result['city'] . ', ' . nl2br($result['address']),
                'address_only' => nl2br($result['address']),
                'postcode' => $result['postcode'],
                'region' => $result['region'],
				'geocode'     => $result['geocode'],
                'geo_latitude' => !empty($geo[0]) ? trim($geo[0]) : '',
                'geo_longitude' => !empty($geo[1]) ? trim($geo[1]) : '',
				'telephone'   => $result['telephone'],
			    'phone' => str_replace(' ', '', $result['fax']),
			    'fax' => $result['fax'],
			    'open'        => nl2br($result['open']),
			    'schedule'    => html_entity_decode($result['schedule']),
			    'google_map_iframe_src' => $result['google_map_iframe_src'],
			    'youtube'   => $result['youtube_id'],
			    'email'     => $result['email'],
        ...
        $this->registry->set('ete_location_list', $data['location_list']);
