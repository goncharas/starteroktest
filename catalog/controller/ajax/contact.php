    ...
    private function getDataByType($type) {        
        ...
        switch($type) {
            case 'repaire': // запрос на ремонт
                ...
                if(!empty($this->request->post['enquiry']) && strpos($this->request->post['enquiry'],'@')){
                  $data['contact_form_receiver'] = empty($this->request->post['enquiry']) ? $this->config->get('config_email') : $this->request->post['enquiry']; // savin 20220501
                }
                break;
