...
if(catalog_controller_mail_order__add){
$log = fopen(DIR_LOGS . 'catalog_controller_mail_order__add.log', 'w');
fwrite($log,' ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
fwrite($log,' $order_info )' . print_r($order_info, true) . '==' . chr(10) . chr(13));
fwrite($log,' $order_status_id )' . print_r($order_status_id, true) . '==' . chr(10) . chr(13));
fwrite($log,' $comment )' . print_r($comment, true) . '==' . chr(10) . chr(13));
fwrite($log,' $notify )' . print_r($notify, true) . '==' . chr(10) . chr(13));
fclose($log);}
...
if(catalog_controller_mail_order__add){
$log = fopen(DIR_LOGS . 'catalog_controller_mail_order__add.log', 'a');
fwrite($log,' $order_products = $this->model_checkout_order->getOrderProducts('.$order_info['order_id'].');' . chr(10) . chr(13));
fwrite($log,' $order_products )' . print_r($order_products, true) . '==' . chr(10) . chr(13));
fclose($log);}
...
if(catalog_controller_mail_order__add){
$log = fopen(DIR_LOGS . 'catalog_controller_mail_order__add.log', 'a');
fwrite($log,' $mail = new Mail($this->config->get(config_mail_engine));   ' . $this->config->get('config_mail_engine') . chr(10) . chr(13));
fwrite($log,' $mail->parameter = $this->config->get(config_mail_parameter);   ' . $this->config->get('config_mail_parameter') . chr(10) . chr(13));
fwrite($log,' $mail->smtp_hostname = $this->config->get(config_mail_smtp_hostname);   ' . $this->config->get('config_mail_smtp_hostname') . chr(10) . chr(13));
fwrite($log,' $mail->smtp_username = $this->config->get(config_mail_smtp_username);   ' . $this->config->get('config_mail_smtp_username') . chr(10) . chr(13));
fwrite($log,' $mail->smtp_password = $this->config->get(config_mail_smtp_password);   ' . $this->config->get('config_mail_smtp_password') . chr(10) . chr(13));
fwrite($log,' $mail->smtp_port = $this->config->get(config_mail_smtp_port);   ' . $this->config->get('config_mail_smtp_port') . chr(10) . chr(13));
fwrite($log,' $mail->smtp_timeout = $this->config->get(config_mail_smtp_timeout);   ' . $this->config->get('config_mail_smtp_timeout') . chr(10) . chr(13));
fclose($log);}
...
if(catalog_controller_mail_order__add){
$log = fopen(DIR_LOGS . 'catalog_controller_mail_order__add.log', 'a');
fwrite($log,' $mail->setTo($order_info[email]);   ' . $order_info['email'] . chr(10) . chr(13));
fwrite($log,' $mail->setFrom($from);   ' . $from . chr(10) . chr(13));
fwrite($log,' $mail->setSender(html_entity_decode($order_info[store_name], ENT_QUOTES, UTF-8));   ' . html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8') . chr(10) . chr(13));
fwrite($log,' $mail->setSubject($subject);   ' . $subject . chr(10) . chr(13));
fwrite($log,' $data )   ' . print_r($data,true) . chr(10) . chr(13));
fwrite($log,' $html )   ' . $html . chr(10) . chr(13));
fclose($log);}
		$mail->setHtml($html);
		$mail->send();
	}
...
if(catalog_controller_mail_order__alert){
$log = fopen(DIR_LOGS . 'catalog_controller_mail_order__alert.log', 'w');
fwrite($log,' ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
fwrite($log,' $route )' . print_r($route, true) . '==' . chr(10) . chr(13));
fwrite($log,' $args )' . print_r($args, true) . '==' . chr(10) . chr(13));
fclose($log);}
		if (isset($args[0])) {
			$order_id = $args[0];
		} else {
		...
		$order_info = $this->model_checkout_order->getOrder($order_id);
if(catalog_controller_mail_order__alert){
$log = fopen(DIR_LOGS . 'catalog_controller_mail_order__alert.log', 'a');
fwrite($log,' $order_info = $this->model_checkout_order->getOrder('.$order_id.');' . '==' . chr(10) . chr(13));
fwrite($log,' $order_info )' . print_r($order_info, true) . '==' . chr(10) . chr(13));
fwrite($log,' $order_status_id )' . print_r($order_status_id, true) . '==' . chr(10) . chr(13));
fclose($log);}
...
fclose($log);}
      ...
      $html = $this->load->view('mail/order_alert', $data);
if(catalog_controller_mail_order__alert){
$log = fopen(DIR_LOGS . 'catalog_controller_mail_order__alert.log', 'a');
fwrite($log,' $html )' . print_r($html, true) . '==' . chr(10) . chr(13));
fclose($log);}
			$mail->setText($html);
			$mail->send();
