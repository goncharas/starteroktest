...
// SAVIN 20220901 end  
	...
	public function add() {
if(catalog_controller_api_cart__add){
$log = fopen(DIR_LOGS . 'catalog_controller_api_cart__add.log', 'w');
...
if(catalog_controller_api_cart__add){
$log = fopen(DIR_LOGS . 'catalog_controller_api_cart__add.log', 'a');
fwrite($log,'if (isset($this->request->post[product])) : foreach ($this->request->post[product] as $product) )' . print_r($product,true) . '==' . chr(10) . chr(13));
fclose($log);}
...
if(catalog_controller_api_cart__add){
$log = fopen(DIR_LOGS . 'catalog_controller_api_cart__add.log', 'a');
fwrite($log,'if (isset($this->request->post[product])) : $this->cart->add('.print_r($product['product_id'],true).', '.print_r($product['quantity'],true).', '.print_r($option,true).', '.print_r($recurring_id,true).', '.print_r($location_id,true).');' . '==' . chr(10) . chr(13));
fclose($log);}

					$this->cart->add($product['product_id'], $product['quantity'], $option, $recurring_id, $location_id);
				}
...
if(catalog_controller_api_cart__add){
$log = fopen(DIR_LOGS . 'catalog_controller_api_cart__add.log', 'a');
fwrite($log,'if (isset($this->request->post[product])) ELSE : $this->cart->add('.print_r($this->request->post['product_id'],true).', '.print_r($quantity,true).', '.print_r($option,true).', '.print_r($recurring_id,true).', '.print_r($location_id,true).');' . '==' . chr(10) . chr(13));
fclose($log);}

					if (!isset($json['error']['option'])) {
						$this->cart->add($this->request->post['product_id'], $quantity, $option, $recurring_id, $location_id);
...
if(catalog_controller_api_cart__products){
$log = fopen(DIR_LOGS . 'catalog_controller_api_cart__products.log', 'w');
fwrite($log,' ' . gmdate('d.m.Y  H:i:s') . '==' . chr(10) . chr(13));
...
if(catalog_controller_api_cart__products){
$log = fopen(DIR_LOGS . 'catalog_controller_api_cart__products.log', 'a');
...
if(catalog_controller_api_cart__products){
$log = fopen(DIR_LOGS . 'catalog_controller_api_cart__products.log', 'a');
fwrite($log,' $stock = ($product[stock_quantity] >= ($product[quantity] - $order_products[$product[product_id]][$product[location_id]][quantity])) ? true : !(!$this->config->get(config_stock_checkout) || $this->config->get(config_stock_warning));' . '==' . chr(10) . chr(13));
...
// SAVIN 20220901 beg   
      if(isset($json['error']['stock']) && $stock_total){
        unset($json['error']['stock']);
      }       
// SAVIN 20220901 end          
