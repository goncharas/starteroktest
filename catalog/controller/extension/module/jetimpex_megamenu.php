...
class ControllerExtensionModuleJetimpexMegaMenu extends Controller
{
	...
	{
if(catalog_controler_extension_module_jetimpex_megamenu__index){
...
fclose($log);}
		...
		$categories = $this->model_catalog_category->getCategories(0, 'c.top=1');
...
fclose($log);}
			...
			foreach ($setting['menu_item'] as $menu_item_id => $menu_item) {
				...
				if ($menu_item['type']) {
					...
					$href       = !empty($menu_item[$this->config->get('config_language_id')]['link']) ? $menu_item[$this->config->get('config_language_id')]['link'] : $menu_item['link'];
                    //$href       = $this->url->link_rewrite($menu_item['link'], true);
...
fclose($log);}
							switch ($column['content']) {
								...
								foreach ($cats_2 as $cat) {
									$list .= "<li>\n<a href=\"" . $this->url->link('product/category', 'path=' . $cat['category_id'], true) . "\">" 
                                            . $cat['name'] 
                                            . "</a>\n</li>\n";
								}
								break;
