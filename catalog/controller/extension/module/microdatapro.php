...
class ControllerExtensionModuleMicrodataPro extends Controller {
    // этот массив можно использовать, если нужно отображать в разметке Организация "@type": "PostalAddress"
    // поскольку в настройках Микродатапро в админке эти поля одноязычные
    // В настоящий момент в разметке Организация нет языковых полей, поэтому используем стандартные настройка
    // а значения для разметки "@type": "Store" на странице Контактов берем из базы (контроллер страницы) через реестр
//    private $contactData = [
//        'ru-ru' => [
//            'name' => 'Стартерок',
//            'streetAddress' => 'ул.Полтавский шлях, 79',
//            'addressLocality' => 'г.Харьков',
//            'addressRegion' => 'Харьковская область',
//            'postalCode' => '61000',
//            'addressCountry' => 'UA',
//            'latitude' => '49.98629824299234',
//            'longitude' => '36.201420422713504',
//            'foundingDate' => '2002',
//        ],       
//        'uk-UA' => [
//            'name' => 'Стартерок',
//            'streetAddress' => 'вул.Полтавський шлях, 79',
//            'addressLocality' => 'м.Харків',
...
//    ];
	...
	public function index() {
        ...
        if(($route == "common/home" || $route == "product/product" || $route == "product/category" 
              || $route == "product/manufacturer/info" || $route == "information/information" 
              || $route == "product/special" || $route == "information/contact") 
              && $b){
		...
		//company data
            ...
            //$this->data['microdata_name']      = $this->contactData[$lang]['name'];
            $this->data['microdata_founding_date']  = '2002';//    = $this->contactData[$lang]['foundingDate'];
            ...
            // данный рейтинг подставляется в блок Organization. По ТЗ его не должно быть.
...
//			$this->data['company_rating_count'] = $c?$this->getAllRating('count'):'true';
        ...
        // search
            $this->data['microdata_search_url'] = $this->url->link('product/search', 'search={search_term_string}');
		...
		//telephones
			if($this->config->get('config_microdata_phones')){
                ...
                $phones = [];
                foreach($microdata_phones as $phoneSet) {
                    $ph = explode("|", $phoneSet);
                    
                    $phones[] = [
                        'phone' => !empty($ph[0]) ? $ph[0] : '',
                        'type' => !empty($ph[1]) ? $ph[1] : '',
                        'email' => !empty($ph[2]) ? $ph[2] : '',
                    ];
                }
                
                $this->data['microdata_phones'] = $phones;
		...
		//product
			if($route == "product/product" && isset($this->request->get['product_id']) && $this->request->get['product_id'] != 0 && $a){
				...
				if($product_info){
					...
					$ipw = $this->config->get('theme_' .$this->config->get('config_theme') . '_image_popup_width');
					$iph = $this->config->get('theme_' .$this->config->get('config_theme') . '_image_popup_height');				
                    ...
                    $this->getReviewsAndRating(['where' => [
...
//                echo '</pre>';
		...
		//category
			if($route == "product/category" && isset($this->request->get['path']) && $this->request->get['path'] != 0 && $b){
				...
				$ipw = $this->config->get('theme_' .$this->config->get('config_theme') . '_image_category_width');
				$iph = $this->config->get('theme_' .$this->config->get('config_theme') . '_image_category_height');				
				$this->data['microdata_popup'] = $this->getImage($this->data['microdata_original_image'], $ipw, $iph);
...
//fwrite($log, '***********' . 'microdata '. date('Y-m-d H:i:s'). '***************');//. chr(13) . chr(13)
//fclose($log);
                $this->getReviewsAndRating(['where' => [
                ...
                if ($this->registry->has('ete_products')) {
                    $results = $this->registry->get('ete_products');
                } else {
                ...
                $this->data['microdata_products_alternative_json'] = $this->products($results);
			}
...
//				$this->data['min_max'] = $this->min_max($results, $manufacturer_id, 'manufacturer');
        ...
        // contact page    
            if ($route == "information/contact" && $a){
                if ($this->registry->has('ete_location_list')) {
                    $eteLocationList = $this->registry->get('ete_location_list');
                    foreach($eteLocationList as $dep) {
                        if ($dep['parent_id'] == 0) {
                            $this->data['microdata_store'][] = $dep;
                        }
                    }
                    //$this->data['microdata_image'] = $contacts['images'];
                }
            }
	...
	public function products($results = array(), $type = 'json', $related = false){
		...
		if($results){
			foreach ($results as $result) {	
				...
				$ipw = $this->config->get('theme_' .$this->config->get('config_theme') . '_image_thumb_width');
				$iph = $this->config->get('theme_' .$this->config->get('config_theme') . '_image_thumb_height');
        ...
        // выборка ревью и расчет рейтинга перенесена в отдельный метод getReviewsAndRating
//        if (!empty($this->data['category_info']['category_id'])) {
//            $this->load->model('catalog/review');
//            $this->data['microdata_reviews'] = $this->model_catalog_review->getLastReviewsByCategory($this->data['category_info']['category_id'], 'r.rating>3');
//        }
//        if (!empty($this->data['manufacturer_info']['manufacturer_id'])) {
//            $this->load->model('catalog/review');
//            $this->data['microdata_reviews'] = $this->model_catalog_review->getLastReviewsByManufacturer($this->data['manufacturer_info']['manufacturer_id'], 'r.rating>3');
//        }
    ...
    protected function getReviewsAndRating($options, $a, $b) {
        ...
        if ($this->config->get('config_review_status')){
            $customWhere = !empty($options['where']['custom']) ? $options['where']['custom']:'';
            
            // если из контроллера передан массив с отзывами, то используем его
            if ($this->registry->has('ete_reviews')) {
                $results = $this->registry->get('ete_reviews');
                $total = sizeof($results);
            }

            // product page
            if (!empty($options['where']['product_id']) && !isset($results)) {
                //$total = (int)$this->model_catalog_review->getTotalReviewsByProductId($options['where']['product_Id']);
                //$results = $this->model_catalog_review->getReviewsByProductId($options['where']['productId'], $a?0:999, $b?9999:0);
                $results = $this->model_catalog_review->getReviews($options);
                $total = sizeof($results);
            }
            // category page
            if (!empty($options['where']['category_id']) && !isset($results)) {
                //d('GET');
                //$total = (int)$this->model_catalog_review->getTotalReviews($options);
                $results = $this->data['microdata_reviews'] = $this->model_catalog_review->getLastReviewsByCategory($options['where']['category_Id'], $customWhere);
                $total = sizeof($results);
            }
            // manufacturer page
            if (!empty($options['where']['manufacturer_id']) && !isset($results)) {
                //$total = (int)$this->model_catalog_review->getTotalReviews($options);
                $results = $this->data['microdata_reviews'] = $this->model_catalog_review->getLastReviewsByManufacturer($options['where']['manufacturer_Id'], $customWhere);
                $total = sizeof($results);
            }
                
            if ($total > 0) {
                // если есть реальные отзывы, считаем средний рейтинг, а сами отзывы отображаем в разметке
                foreach ($results as $result) {
                    $this->data['microdata_reviews'][] = array(
                ...
                // если нет отзывывов, формируем один фейковый отзыв
//                $this->data['microdata_reviews'][] = array(
//                    'author'     => 'Игорь',
//                    'rating'     => 4,
////                    'date_added' => 
...
//                );
        ...
        // данные для блока с рейтингом
        if ($total > 0){
            $this->data['microdata_total_rating_value'] = round((float)($total_rating/$total), 2);
            $this->data['microdata_review_total'] = $total;
        } else {
            // если нет отзывов, то выставляем фейковые значения для усредненного рейтинга
            $this->data['microdata_total_rating_value'] = 4.5;
            $this->data['microdata_review_total'] = 10;
            
            // если нет отзывов - по любой причине, то всегда формируем один фейковый отзыв
            $this->data['microdata_reviews'][] = array(
                    'author'     => 'Игорь',
                    'rating'     => 4,
//                    'date_added' => 
